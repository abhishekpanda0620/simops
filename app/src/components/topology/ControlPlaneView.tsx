...
export function ControlPlaneView({
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-8">
      ...
      {controlPlaneScenario === 'manage-configmap' && (
          <div className="flex justify-center gap-12 items-center">
               ...
               <div className={cn(
                  "p-4 rounded-lg border-2 bg-surface-800 transition-all duration-500 flex flex-col items-center gap-2",
                  ['etcd', 'controller', 'scheduler', 'node-assign', 'complete'].includes(controlPlaneState.phase) 
      ...
      {controlPlaneScenario === 'manage-secret' && (
          <div className="flex justify-center gap-12 items-center">
               {/* Pod */}
               <div className={cn(
                  "p-4 rounded-lg border-2 bg-surface-800 transition-all duration-500 flex flex-col items-center gap-2",
      ...
      {controlPlaneScenario === 'simulate-hpa' && (
          <div className="flex flex-col items-center gap-6 w-full max-w-3xl mx-auto">
               ...
               <div className="flex justify-between w-full">
                   ...
                   <div className={cn(
                       ...
                       ['idle', 'node-flow', 'controller', 'scheduler', 'node-assign', 'complete'].includes(controlPlaneState.phase) 
                       ...
                       <div className="flex items-end gap-2 h-24 bg-surface-900 rounded p-4 relative overflow-hidden">
                            ...
                            {/* Current Bar */}
                            <div 
                              className={cn("w-full bg-primary-500 transition-all duration-1000", controlPlaneState.phase === 'node-flow' || controlPlaneState.phase === 'controller' ? "bg-error-500 h-full" : "bg-success-500 h-1/5")} 
                              style={{ 
                                  height: ['node-flow', 'controller', 'scheduler'].includes(controlPlaneState.phase) ? '100%' : 
                                          ['complete', 'node-assign'].includes(controlPlaneState.phase) ? '45%' : '10%' 
                              }}
                            />
                       </div>
                       <p className="text-xs text-center mt-2 text-surface-300">
                           Current CPU: {['node-flow', 'controller', 'scheduler'].includes(controlPlaneState.phase) ? '200%' : 
                                         ['complete', 'node-assign'].includes(controlPlaneState.phase) ? '45%' : '10%'}
                       </p>
                   </div>
                   ...
                   <div className={cn(
                       ...
                       ['controller', 'scheduler', 'node-assign', 'complete'].includes(controlPlaneState.phase) 
                          ? "border-accent-500 shadow-[0_0_30px_rgba(168,85,247,0.3)] opacity-100" 
                          : "border-surface-700 opacity-30"
                   )}>
                       ...
                       <div className="mt-2 text-xs font-mono bg-surface-900 px-2 py-1 rounded text-accent-400">
                           {['controller', 'scheduler'].includes(controlPlaneState.phase) ? 'Scale: 1 -> 4' : 'Monitoring'}
                       </div>
                   </div>
               </div>
               ...
               <div className="w-full p-4 rounded-lg bg-surface-800/50 border border-surface-700">
                   ...
                   <div className="flex gap-4 flex-wrap justify-center">
                       ...
                       <div className={cn(
                           ...
                           ['node-flow', 'controller', 'scheduler'].includes(controlPlaneState.phase) ? "border-error-500 animate-pulse" : "border-success-500"
                       )}>
                           <Box className={cn("w-8 h-8", ['node-flow', 'controller', 'scheduler'].includes(controlPlaneState.phase) ? "text-error-500" : "text-success-500")} />
                           <span className="text-[10px] text-surface-300">pod-1</span>
                           <div className={cn("text-[10px] px-1 rounded", ['node-flow', 'controller', 'scheduler'].includes(controlPlaneState.phase) ? "bg-error-500/20 text-error-300" : "bg-success-500/20 text-success-300")}>
                               {['node-flow', 'controller', 'scheduler'].includes(controlPlaneState.phase) ? '200% CPU' : '45% CPU'}
                           </div>
                       </div>

                       {/* New Pods (3 more) */}
                       {['scheduler', 'node-assign', 'complete'].includes(controlPlaneState.phase) && [2, 3, 4].map(i => (
      ...
      {controlPlaneScenario === 'simulate-rbac' && (
          <div className="flex flex-col items-center gap-8 w-full max-w-4xl mx-auto">
              ...
              <div className="flex justify-center w-full">
                  <div className={cn(
                      ...
                      controlPlaneState.message?.includes('403') ? "border-error-500 shadow-[0_0_20px_rgba(239,68,68,0.4)]" : "border-primary-500"
                  )}>
                      ...
                      <div className={cn(
                          ...
                          controlPlaneState.message.includes('403') ? "bg-error-500/10 text-error-400" : 
                          controlPlaneState.message.includes('Authorized') || controlPlaneState.phase === 'complete' ? "bg-success-500/10 text-success-400" : "bg-surface-900 text-surface-400"
                      )}>
                          {controlPlaneState.message.includes('403') ? 'Status: 403 Forbidden' : 
                          controlPlaneState.message.includes('Authorized') || controlPlaneState.phase === 'complete' ? 'Status: Authorized' : 'Connecting...'}
                      </div>
                  </div>
              </div>
              ...
              <div className="flex gap-8 justify-center items-center">
                  ...
                  <div className={cn(
                      ...
                      ['kubectl', 'node-assign', 'api-server', 'complete'].includes(controlPlaneState.phase) && !controlPlaneState.message?.includes('403') 
                  ...
                  <div className={cn(
                      ...
                      ['kubectl', 'node-assign', 'api-server', 'complete'].includes(controlPlaneState.phase) && !controlPlaneState.message?.includes('403') && controlPlaneState.message?.includes('Bind')
                        ? "w-24 opacity-100" 
                        : "w-0 opacity-0"
                  )} />
              ...
              {/* Message Display */}
              {controlPlaneState.message?.includes('403') && (
