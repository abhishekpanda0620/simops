...
import type { K8sPod, K8sPVC, K8sStatefulSet, K8sDaemonSet, K8sJob, K8sConfigMap, K8sSecret, K8sHPA } from '@/types';
...
function runScenario(
  ...
  } else if (scenario === 'manage-configmap') {
    return runConfigMapScenario(setState, stopSimulation, actions);
  } else if (scenario === 'manage-secret') {
    return runSecretScenario(setState, stopSimulation, actions);
  } else if (scenario === 'simulate-hpa') {
    return runHPAScenario(setState, stopSimulation, actions);
  }
  return [];
}
...
function runHPAScenario(
  ...
  // 8. Complete
  timeouts.push(setTimeout(() => setState(p => ({ ...p, phase: 'complete', message: 'HPA Scaled Successfully (1 -> 4)' })), 21000));
